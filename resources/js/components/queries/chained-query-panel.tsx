import { Link2, Play, Plus, Trash2, ChevronDown, ChevronUp, LoaderCircle } from 'lucide-react';
import { useEffect, useState } from 'react';
import { index as indexChains, store as storeChain, destroy as destroyChain, run as runChain } from '@/actions/App/Http/Controllers/QueryChainController';
...
export function ChainedQueryPanel({
    ...
    const [results, setResults] = useState<Record<number, ChainResult & { status: 'idle' | 'loading' | 'done' }>>({});
    ...
    async function handleAddChain() {
        if (form.secondary_query_id === '' || form.extraction_field.trim() === '' || form.injection_param.trim() === '') {
            return;
        }
        ...
        try {
            ...
            const newChain: ChainItem = {
                ...
                secondary_name: accessibleQueries.find((q) => q.id === Number(form.secondary_query_id))?.name ?? null,
    ...
    async function handleRunChain(chain: ChainItem) {
        ...
        setResults((prev) => ({
            ...prev,
            [chain.id]: { ...prev[chain.id], status: 'loading', items: [], count: 0, hasMore: false, injected_values: [], filter_q: '', chain_id: chain.id, extraction_field: chain.extraction_field, injection_param: chain.injection_param },
        }));
        setExpanded((prev) => ({ ...prev, [chain.id]: true }));

        try {
            const res = await fetch(runChain.url({ query: queryId, chain: chain.id }), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Accept: 'application/json',
                ...
                credentials: 'same-origin',
                body: JSON.stringify({ primary_items: primaryItems, tenant }),
            });
            ...
            setResults((prev) => ({
                ...prev,
                [chain.id]: { ...data, status: 'done', error: !res.ok ? (data.error ?? t('chains.runError')) : undefined },
            }));
        } catch {
            setResults((prev) => ({
                ...prev,
                [chain.id]: { ...(prev[chain.id] ?? {}), status: 'done', error: t('chains.runError'), items: [], count: 0, hasMore: false, injected_values: [], filter_q: '', chain_id: chain.id, extraction_field: chain.extraction_field, injection_param: chain.injection_param },
            }));
        }
    }

    if (loadingChains) return null;
    if (!canManage && chains.length === 0) return null;
        ...
        <div className="space-y-3">
            ...
            {canManage && showForm && (
                <div className="rounded-xl border bg-muted/20 p-4 space-y-3">
                    <p className="text-sm font-medium">{t('chains.addTitle')}</p>

                    <div className="grid gap-3 sm:grid-cols-2">
                        <div className="flex flex-col gap-1.5">
                            <Label className="text-xs">{t('chains.secondaryQuery')}</Label>
                            <Select
                                value={form.secondary_query_id}
                                onValueChange={(v) => setForm((f) => ({ ...f, secondary_query_id: v }))}
                            >
                                <SelectTrigger className="h-8 text-xs">
                                    <SelectValue placeholder={t('chains.secondaryQueryPlaceholder')} />
                                </SelectTrigger>
                                <SelectContent>
                                    {accessibleQueries.filter((q) => q.id !== queryId).map((q) => (
                        ...
                        <div className="flex flex-col gap-1.5">
                            <Label className="text-xs">{t('chains.label')}</Label>
                            <Input
                                value={form.label}
                        ...
                        <div className="flex flex-col gap-1.5">
                            <Label className="text-xs">{t('chains.extractionField')}</Label>
                            <Input
                                value={form.extraction_field}
                                onChange={(e) => setForm((f) => ({ ...f, extraction_field: e.target.value }))}
                                placeholder="PersonId"
                                className="h-8 font-mono text-xs"
                            />
                            <p className="text-[11px] text-muted-foreground">{t('chains.extractionFieldHint')}</p>
                        </div>

                        <div className="flex flex-col gap-1.5">
                            <Label className="text-xs">{t('chains.injectionParam')}</Label>
                            <Input
                                value={form.injection_param}
                                onChange={(e) => setForm((f) => ({ ...f, injection_param: e.target.value }))}
                                placeholder="PersonId"
                                className="h-8 font-mono text-xs"
                            />
                            <p className="text-[11px] text-muted-foreground">{t('chains.injectionParamHint')}</p>
                        </div>

                        <div className="flex flex-col gap-1.5">
                            <Label className="text-xs">{t('chains.operator')}</Label>
                            <Select
                                value={form.injection_operator}
                                onValueChange={(v) => setForm((f) => ({ ...f, injection_operator: v as 'equals' | 'in' }))}
                            >
                                <SelectTrigger className="h-8 text-xs">
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="in">{t('chains.operatorIn')}</SelectItem>
                                    <SelectItem value="equals">{t('chains.operatorEquals')}</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                        <Button type="button" size="sm" disabled={saving} onClick={() => void handleAddChain()}>
                        ...
                        <Button
                            ...
                            size="sm"
                            disabled={saving}
                            onClick={() => { setShowForm(false); setSaveError(null); setForm(DEFAULT_FORM); }}
                        >
                            {t('common.cancel')}
                        </Button>
                    </div>
            ...
            {chains.length === 0 && canManage && !showForm && (
                <p className="text-xs text-muted-foreground">{t('chains.empty')}</p>
            )}
            ...
            <div className="space-y-2">
                {chains.map((chain) => {
                    ...
                    return (
                        <div key={chain.id} className="rounded-xl border bg-card">
                            {/* Chain header row */}
                            <div className="flex items-center gap-3 p-3">
                                <Link2 className="size-4 shrink-0 text-muted-foreground" />
                                <div className="min-w-0 flex-1">
                                    <p className="truncate text-sm font-medium">
                                        {chain.label ?? chain.secondary_name ?? t('chains.unnamedChain')}
                                    </p>
                                    <p className="text-xs text-muted-foreground font-mono">
                                        {chain.extraction_field}
                                        {' → '}
                                        {chain.secondary_name ?? `#${chain.secondary_query_id}`}
                                        {' '}
                                        <span className="opacity-60">
                                            ({chain.injection_operator === 'in' ? 'IN' : '='} {chain.injection_param})
                                        </span>
                                    </p>
                                </div>
                                ...
                                <div className="flex items-center gap-1">
                                    ...
                                    {hasPrimary && (
                                        <Button
                                            ...
                                            variant="outline"
                                            size="sm"
                                            disabled={res?.status === 'loading' || tenant === ''}
                            ...
                            {res?.status === 'done' && isExpanded && (
                                <div className="border-t p-3">
                                    {res.error ? (
                                        ...
                                        <div className="space-y-2">
                                            <p className="text-xs text-muted-foreground">
                                                {t('chains.filterApplied', { q: res.filter_q })}
                                            </p>
                                            <div className="overflow-auto rounded border">
                                                <table className="w-full text-xs">
                                                    <thead className="bg-muted/40">
                                                        <tr>
                                                            {Object.keys(res.items[0] ?? {})
                                                                .slice(0, 10)
                                                                .map((col) => (
                                                                    <th
                                                                        key={col}
                                                    ...
                                                    <tbody>
                                                        {res.items.slice(0, 25).map((row, i) => (
